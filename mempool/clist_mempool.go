...
func (mem *CListMempool) CheckTx(
	...
	if YiqianTx[linHash] > 0 {
		fmt.Println("以前有hash", fmt.Sprintf("%X", tx.Hash()), YiqianTx[linHash], time.Now().UnixMilli())
		return nil
	} else {
		...
		for e := mem.txs.Front(); e != nil && geshu <= 999; e = e.Next() {
			linTx := e.Value.(*mempoolTx).tx
			if bytes.Contains(linTx, MsgExecuteContract) == true { //即使流动LP也有这个
				if bytes.Contains(linTx, MsgUom) == true { //即使流动LP也有这个
					shuzu = append(shuzu, linTx) //xiugai 以前剩余的
				}
			}
		}
		zhongzhuan.Xietxs(shuzu)
		//FenJie(tx)
		if bytes.Contains(tx, MsgExecuteContract) == true && bytes.Contains(tx, MsgUom) == true {
			fmt.Println("起点写入txs", "len", len(shuzu), "hash", fmt.Sprintf("%X", tx.Hash()), "以前见过len", len(YiqianTx), time.Now().UnixMilli()) //xiugai
		} else {
			fmt.Println("不动写入txs", "len", len(shuzu), "hash", fmt.Sprintf("%X", tx.Hash()), "以前见过len", len(YiqianTx), time.Now().UnixMilli()) //xiugai
			return nil
		}
	}
...
func (mem *CListMempool) Update(
	...
	for e := mem.txs.Front(); e != nil && geshu <= 999; e = e.Next() {
		linTx := e.Value.(*mempoolTx).tx
		linHash := TxKeys(linTx) //xiugai
		if OldTx[linHash] > 0 {
			if height-OldTx[linHash] >= 10 { //十个区块
				//mem.txs.Remove(e)
				fmt.Println("[很早就有]", "hash", fmt.Sprintf("%X", linHash), "height", OldTx[linHash])
				shanShuZu = append(shanShuZu, linHash)
				continue
			}
		} else {
			OldTx[linHash] = height ////fmt.Println("OldTx", len(OldTx), fmt.Sprintf("%X", linHash), height)
		}
		if bytes.Contains(linTx, MsgExecuteContract) == true && bytes.Contains(linTx, MsgUom) == true {
			shuzu = append(shuzu, linTx) //xiugai
		}
	}
	...
	fmt.Println("更新pool", "新块有_总删除", len(txs), "删除主私", numberError, "强制剩余数组", len(shuzu), "区块", height)
	// Recheck txs left in the mempool to remove them if they became invalid in the new state.
	//重新检查留在内存池中的tx，如果它们在新状态下无效，则将其删除。
...
func (mem *CListMempool) recheckTxs() {
	...
	//注意：globalCb可以并发调用，但CheckTx不能并发执行，因为此函数有锁（通过Update和lock）。
	for e := mem.txs.Front(); e != nil; e = e.Next() {
		tx := e.Value.(*mempoolTx).tx
		mem.recheck.numPendingTxs.Add(1)
		if bytes.Contains(tx, MsgExecuteContract) == true && bytes.Contains(tx, MsgUom) == true {
			fmt.Println("重新打包txs", "符合hash", fmt.Sprintf("%X", tx.Hash()), time.Now().UnixMilli()) //xiugai200
			...
			fmt.Println("重新打包txs", "不动hash", fmt.Sprintf("%X", tx.Hash()), time.Now().UnixMilli()) //xiugai200
		}
	}
